      ...
      try {
        ...
        if (record === null) {
          ...
          if (accumulator !== null) {
            ...
            // Render-safe superset of the L1 shape (clients built for L1 records
            // don't choke on absent bond fields — honest empties; ownershipRef is
            // the anchor txid the value-record chain keys off), PLUS notice-window
            // finality. Status reflects the lifecycle: provisional → "immature"
            // (still in its notice window), final → "mature" (owned).
            return writeJson(response, 200, {
              ...accumulator,
              source: "accumulator",
              ownershipRef: accumulator.anchorTxid,
              status: finality?.status === "final" ? "mature" : "immature",
