...
describe("name-state codec", () => {
  ...
  it("encode fails closed on a poison record (bad hex) so corruption can't be persisted", () => {
    const bad = { ...clone(), leafKeyHex: "NOTHEX" } as unknown as NameStateRecord;
    expect(() => encodeNameStateRecord(bad)).toThrow(/cannot encode name-state record/);
  });
  ...
  describe("decode fails closed", () => {
    const cases: ReadonlyArray<readonly [string, unknown]> = [
      ...
      ["uppercase hex (reject-don't-normalize)", { ...clone(), anchoredRoot: "A".repeat(64) }],
