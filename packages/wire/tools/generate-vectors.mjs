...
const descriptorDigest = (d) =>
  // signingProfile enters the digest NORMALIZED (§8.2 never-diverge rule)
...
const valueRecordFile = {
  ...
  vectors: [
    ...
    { id: "value-record-reject-signature-not-hex64", kind: "reject",
      cite: "§8.1 signature(64, owner Schnorr) — wrong JSON type is off-shape",
      envelope: { ...vr, signature: 123 } },
    { id: "value-record-reject-issuedat-not-iso", kind: "reject",
      cite: "§8 timestamps (RFC 3339 UTC; timestamp-form, wire-normative #48)",
      envelope: { ...vr, issuedAt: "not-a-timestamp", signature: hex(vrSig) } },
    { id: "value-record-reject-issuedat-dateparse-legacy", kind: "reject",
      cite: "§8 timestamps (timestamp-form, wire-normative #48) — legacy Date.parse rule retired; free-text date rejects",
      envelope: { ...vr, issuedAt: "June 12, 2026", signature: hex(vrSig) } },
    { id: "value-record-reject-issuedat-offset-form", kind: "reject",
      cite: "§8 timestamps — UTC Z only, numeric offsets reject",
      envelope: { ...vr, issuedAt: "2026-06-12T00:00:00+00:00", signature: hex(vrSig) } },
    { id: "value-record-reject-issuedat-lowercase", kind: "reject",
      cite: "§8 timestamps — uppercase T and Z are literal",
      envelope: { ...vr, issuedAt: "2026-06-12t00:00:00.000z", signature: hex(vrSig) } },
    { id: "value-record-reject-issuedat-short-fraction", kind: "reject",
      cite: "§8 timestamps — fraction absent or exactly three digits",
      envelope: { ...vr, issuedAt: "2026-06-12T00:00:00.12Z", signature: hex(vrSig) } },
    { id: "value-record-reject-issuedat-impossible-date", kind: "reject",
      cite: "§8 timestamps — real calendar instant: Date.parse rolls 2026-02-30 to Mar 2, the round-trip must not",
      envelope: { ...vr, issuedAt: "2026-02-30T00:00:00Z", signature: hex(vrSig) } },
    { id: "value-record-reject-sequence-above-bound", kind: "reject",
      cite: "§8 sequence bound (sequence-bound, wire-normative #48) — 2^53 rejects, 2^53−1 is the ceiling",
      envelope: { ...vr, sequence: 2 ** 53, signature: hex(vrSig) } },
    { id: "value-record-reject-unassigned-valuetype", kind: "reject",
      cite: "§8.1 valueType outside the registry MUST be rejected (reserved, fail closed)",
      envelope: { ...vr, valueType: 0x03, signature: hex(vrSig) } },
...
const descriptorFile = {
  ...
  vectors: [
    ...
    { id: "descriptor-valid-noncanonical-profile-input", kind: "valid",
      ...
      envelope: { ...rd, signingProfile: " BIP322 ", signature: hex(rdSig) },
      digest: hex(rdDigest), signerXOnlyPubkey: hex(owners[0].pub),
      normalizedProfile: "bip322",
      note: "same digest and signature as descriptor-valid; only the JSON rendering differs" },
    { id: "descriptor-reject-signature-not-hex64", kind: "reject",
      cite: "§8.2 signature(64, owner Schnorr) — wrong JSON type is off-shape",
      envelope: { ...rd, signature: 123 } },
    { id: "descriptor-reject-issuedat-not-iso", kind: "reject",
      cite: "§8 timestamps (RFC 3339 UTC; timestamp-form, wire-normative #48)",
      envelope: { ...rd, issuedAt: "not-a-timestamp", signature: hex(rdSig) } },
    { id: "descriptor-reject-issuedat-offset-form", kind: "reject",
      cite: "§8 timestamps (timestamp-form, wire-normative #48) — shared preamble rule, §8.2 envelope",
      envelope: { ...rd, issuedAt: "2026-06-12T00:00:00+00:00", signature: hex(rdSig) } },
    { id: "descriptor-reject-sequence-above-bound", kind: "reject",
      cite: "§8 sequence bound (sequence-bound, wire-normative #48) — §8.2 envelope",
      envelope: { ...rd, sequence: 2 ** 53, signature: hex(rdSig) } },
...
const walletProofFile = {
  ...
  vectors: [
    ...
    { id: "wallet-proof-reject-bip322-malformed-signature", kind: "reject",
      cite: "§8.3 BIP322 verification — base64-shaped but structurally invalid witness bytes are an INVALID signature (verify false), never a verifier crash",
      envelope: { ...wpNoTip.envelope, signatureBase64: "AAAA" } },
    { id: "wallet-proof-reject-signature-not-base64-string", kind: "reject",
      cite: "§8.3 signatureBase64 — wrong JSON type is off-shape",
      envelope: { ...wpNoTip.envelope, signatureBase64: 123 } },
