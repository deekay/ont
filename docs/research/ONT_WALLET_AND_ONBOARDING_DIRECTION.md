...
- **Lightning credential:** Lexe's model (enclave + phone, with a convenience-grade encrypted Google
  Drive backup) governs only the **$1 payment leg**. Fine for that — a Google lockout there is a
...
How the user pays the publisher their ~$1 gate, by tier of trust-minimization:
...
1. **Now / launch — naive pay to a *reputable* publisher.** Early publishers are known, always-online
   ...
   analysis). For a **$1** stake, **reputation + the L1 fallback + a rare absorbed $1** is good enough;
...
it from the critical path**: (a) reputable always-online publishers make naive-pay fine for $1 at
