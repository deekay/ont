...
1. **✅ T2 resolved (2026-05-23):** user-cost target set to ~$1, revisit on feedback. (Was the lone 🚫.)
...
| **S2 Neutral scarce gate** | ✅ | $1 gate (miner-fee or PoW), locally verifiable, no issuer. |
...
| **S6 Anti-squat cost one-time/sunk** | ✅ | $1 one-time sunk; bonds returnable. No recurring/contestable squat cost. |
...
| **A2 Low cost for ordinary names** | ✅ (spirit) | $1 is cheap for mass use — but see T2 for the quantified conflict. |
| **A3 Simplicity** | 🟡 | *User* story is simple ("claim for $1, own forever"); the *verifier/mechanism* story (accumulator + DA agreement + windows + two rails + auction) is not. Acceptable under §11 (A3 is the lowest-priority flex), but real. |
...
| **T2 User cost (~$1 target)** | ✅ (resolved 2026-05-23) | T2 revised from "cents-scale" to **~$1 denominated in bitcoin** — accepted for human mass adoption; revisit toward cents only if feedback or machine/IoT-at-billions pushes back. The earlier conflict is closed by setting the target to the design. |
...
| --- | --- | --- |
| **Squatter** | ✅ | Linear sunk cost ($1 × N); premium names → auction. 1M names ≈ $1M. |
...
1. **✅ T2 vs the $1 gate — resolved (2026-05-23):** target set to ~$1, revisit on feedback. Machine/IoT-at-billions (which would want ~$0.10) deferred until it pushes back.
