...
| **live-enforcement** (LE-INDEX + LE-RESOLVE) | the indexer service runs the audited enforcement over *ingested* anchors and writes per-name state; the resolver serves that enforced state + the evidence trace | green, hermetic |
...
| Live enforcement loop (LE-INDEX + LE-RESOLVE) | **Wired (hermetic)** | Indexer runs `enforceBatchedClaim` over ingested anchors → per-name `@ont/name-state-store`; resolver serves the enforced state + trace (stamped not-ownership-authority). Hermetic e2e. Not on a live network. |
